...
public class ClientAPP {
	...
	public static void main(String[] args) {
		...
		double lat = 41.275745;
		double longitude = 1.987273;
		try{
		Thread.sleep(1500);
		}catch(Exception e){
		...
		try {
			...
			lat = lat + Math.pow(-1, random2)*0.01*Math.random();
			longitude = longitude + Math.pow(-1, random3)*0.002*Math.random();
					...
					try {
						...
						if (data.length == 2) {
							...
							Thread thread = new Thread() {
								public void run() {
									while (ClientAPP.working) {
										ClientAPP.Update();
										int tiempo = (int) (5000+5000 * Math.random());
										try {
											Thread.sleep(tiempo);
